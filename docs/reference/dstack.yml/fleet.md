...
=== "Backend fleet"
    ...
    ###### `idle_duration` - (Optional) `bool | int | str | "off"` Time to wait before terminating idle instances. Only instances above `nodes.min` are terminated, so a fleet with a fixed `nodes: N` keeps its instances. Defaults to `5m` for runs and `3d` for fleets. Use `off` for unlimited duration. { #idle_duration data-toc-label='idle_duration' class='reference-item' }
