...
###### `baseline` - (Optional) `bool` Whether the first trial must be a baseline that serves the model with the serving framework's recommended defaults instead of an optimization attempt. Defaults to `false`. { #baseline data-toc-label='baseline' class='reference-item' }
